    ...
    <div class="card">
      <div class="card-body">
        <div class="d-flex justify-content-between align-items-center flex-wrap">
          <h4 class="card-title mb-2">Sửa đặt bàn</h4>
        ...
        <form action="{{ route('admin.reservations.edit.process', $reservation->id) }}" method="POST">
          @csrf
          ...
          <div class="form-group">
            ...
            <input type="number" name="no_guest" class="form-control" value="{{ old('no_guest', $reservation->no_guest) }}" min="1" max="100" required>
          </div>
          ...
          <div class="form-group">
            <label>Giờ</label>
            <input type="time" name="time" class="form-control" value="{{ old('time', $reservation->time) }}" required>
          </div>

          <div class="form-group">
            <label>Ghi chú</label>
            <textarea name="message" class="form-control" rows="4">{{ old('message', $reservation->message) }}</textarea>
          </div>
        ...
        <script>
          (function () {
            const form = document.querySelector('form[action="{{ route('admin.reservations.edit.process', $reservation->id) }}"]');
            if (!form) return;
